...
__device__ void sha256_hash(const unsigned char* data, unsigned int length, unsigned char* result)
{
  unsigned int h[8] = {
...
int main()
{
  ...
  std::vector<int> offsets(message_count);
  std::vector<int> lengths(message_count);
  std::vector<unsigned char> h_messages;
  int total_bytes = 0;
  for (int i = 0; i < message_count; i++)
  {
      lengths[i] = (int)messages[i].size();
      offsets[i] = total_bytes;
      total_bytes += lengths[i];
      h_messages.insert(h_messages.end(), messages[i].begin(), messages[i].end());
  }
  ...
  cudaMalloc(&d_offsets,  message_count * sizeof(int));
  cudaMalloc(&d_lengths,  message_count * sizeof(int));
  cudaMalloc(&d_digests,  message_count * 32);

  cudaMemcpy(d_messages, h_messages.data(), total_bytes, cudaMemcpyHostToDevice);
  cudaMemcpy(d_offsets,  offsets.data(),  message_count * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(d_lengths,  lengths.data(),  message_count * sizeof(int), cudaMemcpyHostToDevice);

  int threadsPerBlock = 256;
  int numBlocks = (message_count + threadsPerBlock - 1) / threadsPerBlock;

  sha256_kernel<<<numBlocks, threadsPerBlock>>>(d_messages, d_offsets, d_lengths,
                                                  d_digests, message_count);

  cudaDeviceSynchronize();
  std::cout << "kernel status: " << cudaGetErrorString(cudaGetLastError()) << "\n\n";

  std::vector<unsigned char> h_digests(message_count * 32);
  cudaMemcpy(h_digests.data(), d_digests, message_count * 32, cudaMemcpyDeviceToHost);

  bool all_ok = true;
  for (int i = 0; i < message_count; i++)
  {
      // Build the 64-char hex string with streams instead of sprintf.
      std::ostringstream hex;
      for (int j = 0; j < 32; j++)
          hex << std::hex << std::setw(2) << std::setfill('0')
              << (int)h_digests[i*32 + j];
      std::string got = hex.str();

      bool ok = (got == expected[i]);
      if (!ok) all_ok = false;
      std::cout << "msg " << i << " (\"" << messages[i] << "\")\n"
                << "  got: " << got << "\n"
                << "  exp: " << expected[i] << "  [" << (ok ? "PASS" : "FAIL") << "]\n\n";
  }
  std::cout << (all_ok ? "ALL PASS\n" : "SOME FAILED\n");
